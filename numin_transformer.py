"""
Numin2 with Transformer-based MAML

Uses Transformer for time series encoding instead of LSTM.
"""
...
class TransformerEncoder(nn.Module):
    def __init__(self, input_dim=50, hidden_dim=256, num_heads=4, num_layers=3):
        super().__init__()
...
class NuminTransformer(nn.Module):
...
def main():
    ...
    parser.add_argument('--save_dir', type=str, default='checkpoints_numin_transformer')
    ...
    model = NuminTransformer(num_stocks=50, hidden_dim=args.hidden_dim)
    print(f"Parameters: {sum(p.numel() for p in model.parameters()):,}")

    maml = NuminTransformerMAML(model, args.inner_lr, args.outer_lr, args.inner_steps, device)
